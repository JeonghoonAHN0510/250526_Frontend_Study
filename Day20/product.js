...
// 재고 입고 폼: 제품 ID, 변동 유형 , 변동 수량, 입출사유 를 입력할 수 있는 <input>, <textarea>, <button> 태그 포함.
...
// 요구사항에 맞게 데이터모델링 과 샘플 데이터를 구성합니다.

// 재고 입고 (Inventory Inflow / Stock In)
// 기능: 특정 제품의 재고를 증가시키기 위한 입고 내역 기록.
// 입력 필드: 제품 ID (필수), 입고 수량 (필수, 양의 정수), 입고 날짜 (기본값 오늘, 변경 가능), 비고 (선택).
// 동작: 제품 ID 유효성 검사, 수량 유효성 검사, 성공/실패 메시지, 해당 제품 재고 증가, 재고 로그 기록 추가.

// 전체 재고 로그 조회 (View All Inventory Logs)
// 기능: 시스템에 기록된 모든 재고 변동 내역을 표 형태로 조회.
// 표시 항목: 로그 ID, 제품 ID, 제품명, 변동 유형 ('입고/출고'), 변동 수량, 변동 날짜/시간, 비고.
// 동작: 페이지 로드/버튼 클릭 시 목록 표시, 로그 없을 시 메시지 표시.
...
// [ 요구사항5 ]
// "2단계와 4단계에서 구현된 관리자 페이지의 제품 및 재고 로그 데이터를 localStorage에 저장하고 불러와
// 새로고침 시에도 데이터가 유지되도록 구현합니다.
