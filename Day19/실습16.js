...
function signup(){
    ...
    for ( let i = 0; i <= memberList.length - 1; i++){
        const JSONobj = JSON.stringify( memberList );
        sessionStorage.setItem('memberList', JSONobj );        
    }
    alert('회원 등록 성공');                            // 성공 알림
}
...
function login(){
    ...
    let JSobj = JSON.parse( sessionStorage.getItem('memberList') );
    console.log( JSobj );
    for ( let i = 0; i <= JSobj.length - 1; i++){
        if ( JSobj[i].id == obj.id && JSobj[i].pw == obj.pw ){
            alert('로그인 성공')
            return;
        }   // else 사용하면 안 됨
    }
...
function signup(){
    ...
    // 3. 입력값 객체화
    //===================== sessionStorage에서 memberList 가져오기 =========================
    let no = 1; // 회원번호 초기값
        // 1) sessionStorage에서 memberList 가져오기
        let memberList = sessionStorage.getItem('memberList') 
        // 2) memberList가 존재하지 않으면 새로 생성, 존재하면 타입 변환
        if ( memberList == null ){                      // 존재하지 않으면
            memberList = [];                            // 새로운 배열 생성 / 회원번호 초기값 사용
        }else {                                         // 존재하면
            memberList = JSON.parse( memberList );      // JSON(배열타입)으로 변환하기
    ...
    alert('회원 등록 성공');                            // 성공 알림
    //===================== sessionStorage에 memberList 저장하기 =========================
        // 1) 배열 타입을 JSON문자열 타입으로 변환
        let jsonData = JSON.stringify(memberList);
        // 2) sessionStorage에 memberList 속성명으로 배열 저장하기
...
function login(){
    ...
    // 1. 입력 마크업 객체 가져오기
    const loginId = document.querySelector('.loginId');     // console.log(signId)
    const loginPw = document.querySelector('.loginPw');     // console.log(signPw)
    // 2. 입력 마크업 객체의 입력값 가져오기
    const id = loginId.value;                               // console.log( id );
    const pw = loginPw.value;                               // console.log( pw );
    //===================== sessionStorage에서 memberList 가져오기 =========================
    let memberList = sessionStorage.getItem('memberList');  // memberList라는 속성값 가져오기
    if ( memberList == null ){                          // 존재하지 않으면
        memberList = [];                                // 새로운 배열 생성
    }else {                                             // 존재하면
        memberList = JSON.parse( memberList );          // JSON(배열 타입)으로 변환
    }    
    // 3. 회원목록 내에서 입력값과 일치한 정보 찾기(비교)
    for( let i = 0; i <= memberList.length - 1; i++){
