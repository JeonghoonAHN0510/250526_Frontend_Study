...
function func1(){
    // 함수 실행 확인 로그
    console.log('----func1----')
    // 1. input 마크업 객체 값 가져오기
    const id = document.querySelector('#idInput').value;
    // 가져온 값 확인
    console.log( id );
    // 2. 입력받은 값을 localstorage에 저장
    localStorage.setItem('id', id);     // 속성명 'id'로 id 속성값을 저장
    // localStorage 확인
...
// 2. 변환한 문자열을 localStorage에 저장
localStorage.setItem( 'data', data );        // 자료에 속성명을 부여하여 저장
console.log( localStorage );
...
localStorage.removeItem('data');     // ex2 특정 데이터 삭제
console.log( localStorage );
localStorage.clear();               // localStorage 전체 데이터 삭제
console.log( localStorage );
...
if ( visit == null ){   // 속성명이 존재하지 않으면 속성 초기화 해주기
    // 2. 새로운 속성 만들기
    sessionStorage.setItem( 'visit', 1 );
    visit = 1;
}else {                 // 이미 존재하는 속성명이면
    visit = Number( visit ) + 1; // 1을 증가시킨다.
